...
<%
    ...
    String noticeWriter = ""; // 작성자는 세션 등에서 가져와야 함
    String mainNoticeImage = ""; // 대표 이미지 (썸머노트 내용 중 첫 번째 이미지 또는 별도 업로드)

    try {
        MultipartRequest multi = new MultipartRequest(request, savePath, maxSize, encType, new DefaultFileRenamePolicy());

        // 1. 폼 데이터 가져오기
        noticeTitle = multi.getParameter("title"); // HTML form의 name="title"
        noticeContent = multi.getParameter("content"); // HTML form의 name="content"
        ...
        if (matcher.find()) {
            mainNoticeImage = matcher.group(1);
        ...
        dto.setNotice_title(noticeTitle);
        dto.setNotice_content(noticeContent);
        dto.setNotice_img(mainNoticeImage); // 추출한 첫 이미지 또는 대표 이미지 파일명
        dto.setNotice_readcount(0); // 새 글이므로 조회수는 0
        dto.setNotice_writer(noticeWriter);
        ...
        String redirectUrl = request.getContextPath() + "/index.jsp?main=board/boardList.jsp&sub=notice.jsp";
        out.println("<script>alert('공지사항 글이 성공적으로 작성되었습니다.'); location.href='" + redirectUrl + "';</script>");
